...
| `graph.sdk.plugin_store` | `__init__._data_dir` | Resolves the instance-scoped durable store. A missing or failed host seam raises instead of silently crossing into a global home-directory fallback. |
...
| `register_watch_hook` | `goals.py` | When a `/learn` watch trips (target strength reached), the hook retires the loop — `stop_goal_loop` for sugar-armed loops, a hand `cancel_scheduled` for composed ones — and emits `goal_achieved`. Both watch-id schemes recognized. |
| `graph.sdk.start_goal_loop` (fallback: `create_watch` + `schedule_recurring`) | `commands.py` `/learn` | The self-driving loop in one call (protoAgent #2061 — an API this plugin's seam audit got reinstated): watch + tick under a shared id, idempotent by `(plugin_id, loop_id)`, rolled back together. **Feature-detected**, not version-gated: older hosts get the identical loop composed from the two primitives — a plugin shouldn't raise `min_protoagent_version` for sugar. |
